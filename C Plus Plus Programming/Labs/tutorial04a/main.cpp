...
void Parse(ifstream &fileIn, ofstream &fileOut, string &attribute, int &level, int &maxSize, int &levelOfMaxSizeObject)
{
	if (fileIn >> attribute)
	{
		if (attribute.find("{") != string::npos)
		{
			...
			Parse(fileIn, fileOut, attribute, level, maxSize, levelOfMaxSizeObject);
