...
List2d::Node::Node(List item) : m_item(item)
{
...
List2d::Node::Node(List item, Node *nextNode, Node *previousNode) : m_nextNode(nextNode), m_previousNode(previousNode), m_item(item)
{
...
//Constructor
List2d::List2d() : m_headNode(0), m_numberOfElements(0)
{
...
void List2d::AddNode(List item, int index)
{
	if (m_numberOfElements++ > 0)   // Check if the List is currently empty
	{
...
void List2d::RemoveNode()
{
	Node *temporaryNode = m_headNode;

	temporaryNode->GetItem().ClearList();
...
List2d::Node *List2d::GetElement(int index)
{
