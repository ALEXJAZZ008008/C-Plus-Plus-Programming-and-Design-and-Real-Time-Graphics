...
void EyeAnimation(inout VertexInput input)
{
	if (ObjectState > 0.0f)
	{
		float translationMagnitude = 0.0f;
...
void Animation(inout VertexInput input)
{
	...
	{
		if (ObjectType > 1.0f && ObjectType < 3.0f)
		{
			EyeAnimation(input);
		}
		else
...
PixelInput ModelVertexShader(VertexInput input)
{
	Animation(input);
	...
	output.ViewPosition = normalize(ViewPosition - output.WorldPosition);

	output.RenderState = RenderState;

	output.Normal = normalize(mul(float4(input.Normal, 1.0f), WorldMatrix).xyz);

	if (RenderState > 2.0f)
	{
		output.Tangent = normalize(mul(float4(input.Tangent, 1.0f), WorldMatrix).xyz);
		output.Binormal = normalize(mul(float4(input.Binormal, 1.0f), WorldMatrix).xyz);
	}
